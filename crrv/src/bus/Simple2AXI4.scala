...
/** 从Simple接口到AXI 4接口的转换器 AXI4的地址宽度被定位 32 位 地址通道通过重新发送，解决了地址变化问题
  */
class Simple2AXI4 extends Module {
  val io = IO(new Bundle {
    val simple = Flipped(new SimpleIO(ADDR_WIDTH, DATA_WIDTH))
    val axi    = new AXI4MasterIO(ADDR_WIDTH, 32)
  })
  // 地址变换
  val simple_addr = io.simple.out.bits.addr
  ...
  val wstrb = MuxLookup(dataSize, 0.U)(
    Seq(
      0.U -> ("b0001".U(4.W) << addr(1, 0)),
      1.U -> ("b0011".U(4.W) << addr(1, 0)),
      2.U -> ("b1111".U(4.W) << addr(1, 0))
    )
  )
